-------------------------------------------------------------------------#
--任务名：dwd_ast_exch_hldp_ods_mts_kgdbrzrq_shares.q
--目标表：dwd_ast_exch_hldp 场内持仓
--源表：ods_mts_kgdbrzrq_shares
--运行频度：每日
--任务功能说明：
--作者：xujianliang
--创建日期：20220829
...
--删除数据
alter table rtassets_dw.dwd_ast_exch_hldp drop if exists partition(part_ymd = '${batch_date}' , ssys_tab = 'ods_mts_kgdbrzrq_shares');

insert into table rtassets_dw.dwd_ast_exch_hldp partition(part_ymd='${batch_date}',ssys_tab='ods_mts_kgdbrzrq_shares')
( 
   data_date               --DATA_DATE
  ,etl_time                --ETL_TIME
  ,busi_date               --业务日期
...
select '${batch_date}'                                       as data_date                   --data_date
       ...
       ,concat('ast','|','220','|',cast(t1.account as string),'|',t1.secu_acc,'|',cast(t1.secu_intl as string),'|',t1.seat,'|',cast(t1.ext_inst as string))    as ast_num                     --资产编码
       ,'2'                                                           as ast_type_code                 --资产类型代码
       ,'220'                                                         as hldp_type_code                --持仓类型代码
...
from (select * from rtassets_ods.ods_mts_kgdbrzrq_shares where part_ymd='${batch_date}') t1
left join
