-------------------------------------------------------------------------#
-- 任务名：       dwd_ast_cptl_ods_cts_kgdb_capital.sql
-- 目标表：       dwd_ast_cptl   资金
-- 源表：         ods_cts_kgdb_capital
-- 运行频度：     每日
-- 任务功能说明：资金
-- 作者：        xujianliang
-- 创建日期：    20220825
...
--删除数据
alter table rtassets_dw.dwd_ast_cptl drop if exists partition (part_ymd = '${batch_date}' ,ssys_tab = 'ods_cts_kgdb_capital');

insert into table rtassets_dw.dwd_ast_cptl partition(part_ymd='${batch_date}',ssys_tab='ods_cts_kgdb_capital')
(  data_date         --DATA_DATE
  ,etl_time          --ETL_TIME
  ,busi_date         --业务日期
  ,ast_num           --资产编码
...
select '${batch_date}'                                       as data_date              --DATA_DATE
     ...
     ,concat('ast','|','110','|',cast(t1.account as string),'|',t1.currency) as ast_num                --资产编码
     ,'1'                                                    as ast_type_code          --资产类型代码
     ,'110'                                                  as cptl_type_code         --资金类型代码
...
from (select * from rtassets_ods.ods_cts_kgdb_capital where part_ymd='${batch_date}') t1
left join
