-------------------------------------------------------------------------#
-- 任务名：       dwd_agt_repo_contr_ods_cts_kgdb_xy_contracts.q
-- 目标表：       dwd_agt_repo_contr
-- 源表：         rtassets_ods.ods_cts_kgdb_xy_contracts
-- 运行频度：     每日
-- 任务功能说明：回购合约 -3
-- 作者：        xujianliang
-- 创建日期：    20220902
...
--删除数据
alter table rtassets_dw.dwd_agt_repo_contr drop if exists partition(part_ymd = '${batch_date}' , ssys_tab = 'ods_cts_kgdb_xy_contracts');

-- 更新数据
insert into table rtassets_dw.dwd_agt_repo_contr partition (part_ymd = '${batch_date}' , ssys_tab = 'ods_cts_kgdb_xy_contracts') 
(
 data_date             --DATA_DATE
,etl_time              --ETL_TIME
,contr_num             --合约编号
...
select    
    ...
    ,concat('agt','|','4','|',cast(t1.trd_date as string),'|',t1.market,'|',t1.constr_sn,'|',t1.constr_type,'|',t1.secu_acc)    as contr_num      --合约编号
    ,'4'                                            as contr_type_code            --合约类型代码
    ...
    ,cast(t1.secu_intl as decimal(28,6))    as pd_incd                 --产品内码
    ...
    ,concat(cast(t1.trd_date as string),'|',t1.market,'|',t1.constr_sn,'|',t1.constr_type,'|',t1.secu_acc)       as ssys_num          --来源系统编码
from (select * from rtassets_ods.ods_cts_kgdb_xy_contracts where part_ymd='${batch_date}') t1     
left join
