-------------------------------------------------------------------------#
-- 任务名：       dwd_agt_repo_contr_ods_cts_kgdb_bb_contracts.sql
-- 目标表：       dwd_agt_repo_contr
-- 源表：         rtassets_ods.ods_cts_kgdb_bb_contracts
-- 运行频度：     每日
-- 任务功能说明：回购合约 -1
-- 作者：        xujianliang
-- 创建日期：    20220902
...
--删除数据
alter table rtassets_dw.dwd_agt_repo_contr drop if exists partition(part_ymd = '${batch_date}' , ssys_tab = 'ods_cts_kgdb_bb_contracts');

-- 更新数据
insert into table rtassets_dw.dwd_agt_repo_contr partition (part_ymd = '${batch_date}' , ssys_tab = 'ods_cts_kgdb_bb_contracts') 
(
 data_date             --DATA_DATE
,etl_time              --ETL_TIME
,contr_num             --合约编号
...
select                                                                     
    ...
    ,concat('agt','|','1','|',cast(t1.cust_code as string),'|',cast(t1.trd_date as string),'|',t1.trd_id,'|',t1.market,'|',t1.board,'|',t1.matched_sn)            as contr_num                  --合约编号
    ,'1'                                                         as contr_type_code            --合约类型代码
    ...
    ,t1.perform_flag       as guar_flag             --履约标志
    ,cast(t1.perform_amt   as decimal(28,6))    as guar_amt              --履约金额
    ,cast(t1.share_trd_frz as decimal(28,0))    as share_trade_frz_cnt   --股份交易冻结数
    ,cast(t1.trd_frz       as decimal(28,6))    as trade_frz_amt         --交易冻结金额
    ...
    ,concat(cast(t1.cust_code as string),'|',cast(t1.trd_date as string),'|',t1.trd_id,'|',t1.market,'|',t1.board,'|',t1.matched_sn)       as ssys_num                   --来源系统编码
from (select * from rtassets_ods.ods_cts_kgdb_bb_contracts where part_ymd='${batch_date}') t1
left join
