-------------------------------------------------------------------------#
-- 任务名：       dwd_ast_cptl_ods_cts_kgdb_hkex_capital.sql
-- 目标表：       dwd_ast_cptl   资金
-- 源表：         ods_cts_kgdb_hkex_capital
-- 运行频度：     每日
-- 任务功能说明：
-- 作者：        xujianliang
-- 创建日期：    20220913
...
--删除数据
alter table rtassets_dw.dwd_ast_cptl drop if exists partition (part_ymd = '${batch_date}' ,ssys_tab = 'ods_cts_kgdb_hkex_capital');

insert into table rtassets_dw.dwd_ast_cptl partition(part_ymd='${batch_date}',ssys_tab='ods_cts_kgdb_hkex_capital')
(  data_date         --DATA_DATE
  ,etl_time          --ETL_TIME
  ,busi_date         --业务日期
  ,ast_num           --资产编码
...
select '${batch_date}'                                       as data_date                  --DATA_DATE
     ...
     ,concat('ast','|','111','|',cast(t1.account as string),'|',t1.currency) as ast_num    --资产编码
     ,'1'                                                    as ast_type_code              --资产类型代码
     ,'111'                                                  as cptl_type_code             --资金类型代码
     ...
     ,null                                                   as acct_bal                   --账户余额
     ,cast(t1.available  as decimal(24,6))                   as aval_amt                   --可用金额
     ,cast(t1.frz_sumavl as decimal(24,6))                   as frz_amt                    --冻结金额
     ...
     ,null                                                   as dep_int_aggr               --存款利息积数
     ,null                                                   as dep_int                    --存款利息
     ,t1.status                                              as cptl_state                 --资金状态
     ,'cts'                                                  as ssys_code                  --系统来源代码
     ,cast(t1.account as string)                             as ssys_num                   --系统来源编码
from (SELECT * from rtassets_ods.ods_cts_kgdb_hkex_capital where part_ymd='${batch_date}') t1
left join
